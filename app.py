...
st.markdown("""
    ...
    .welcome-box { text-align: center; padding: 40px; border-radius: 25px; background: #f0f4f8; border: 2px solid #00acc1; }
...
if st.session_state.page == "login":
    if st.session_state.is_finished:
        st.warning(f"عذراً {st.session_state.user_name}، لقد أتممتِ الاختبار مسبقاً ولا يمكن الإعادة حالياً.")
    else:
        ...
        if st.button("تسجيل ودخول 🚀"):
            if name:
                ...
                st.error("يرجى إدخال الاسم للمتابعة")
        st.markdown("</div>", unsafe_allow_html=True)

# --- 2. الصفحة الترحيبية ---
elif st.session_state.page == "welcome":
    st.markdown(f"<div class='welcome-box'><h2>مرحباً المبدعة {st.session_state.user_name}</h2>", unsafe_allow_html=True)
    st.write("أنا LAI، ذكاؤكِ الاصطناعي المساعد. سنبدأ الآن رحلة تقييم 25 مهارة علمية.")
    if st.button("بدء الاختبار التفاعلي 🏁"):
        st.session_state.page = "quiz_لغتي"
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)

# --- 3. بنك الأسئلة (5 لكل مادة) ---
quiz_data = {
    "لغتي": [("علامة رفع الأسماء الخمسة؟", ["الواو", "الألف"]), ("أسلوب 'ما أجمل السماء'؟", ["تعجب", "نداء"]), ("همزة 'اسم'؟", ["وصل", "قطع"]), ("جمع 'قلم'؟", ["أقلام", "قلمون"]), ("الفاعل يكون؟", ["مرفوعاً", "منصوباً"])],
    ...
    "علوم": [("رمز الأكسجين؟", ["O", "H"]), ("تحول السائل لغاز؟", ["تبخر", "تجمد"]), ("مركز المجموعة الشمسية؟", ["الشمس", "الأرض"]), ("قوة الجذب؟", ["الجاذبية", "الاحتكاك"]), ("تتنفس الأسماك بـ؟", ["الخياشيم", "الرئة"])]
}

subjects = list(quiz_data.keys())
for i, sub in enumerate(subjects):
    if st.session_state.page == f"quiz_{sub}":
        st.markdown(f"### 📚 مادة {sub} - (الطالبة: {st.session_state.user_name})")
...
if st.session_state.page == "final_report":
    ...
    st.markdown(f"<div class='report-card'><h3>🌟 نقطة القوة: {strongest}</h3><a href='https://ien.edu.sa/'>🔗 محتوى إثرائي</a></div>", unsafe_allow_html=True)
    st.markdown(f"<div class='report-card' style='border-color: #f44336;'><h3>📉 مادة للتطوير: {weakest}</h3><a href='https://ien.edu.sa/' style='color:#f44336;'>📚 مراجعة عين</a></div>", unsafe_allow_html=True)

    st.info("💡 **نصيحة LAI:** ابدأي يومك بمراجعة المادة الأقل درجة لتتحول لنقطة قوة!")
    st.markdown("### 📅 جدول المذاكرة")
    st.table({"اليوم": ["الأحد", "الاثنين", "الثلاثاء"], "التركيز": [f"مراجعة {weakest}", f"تمارين {weakest}", f"إبداع {strongest}"]})

    if st.button("الذهاب إلى مُبتكر 💡"):
        st.session_state.page = "innovator"
        st.rerun()

# --- 5. صفحة مُبتكر وقفل الإعادة ---
if st.session_state.page == "innovator":
    st.session_state.is_finished = True # تفعيل قفل الإعادة
    st.markdown("<div class='innovation-box'>", unsafe_allow_html=True)
    st.title("💡 مختبر مُبتكر للريادة")
    with st.form("innovator_form"):
        st.markdown(f"أهلاً بالمبتكرة {st.session_state.user_name}، ما هو مجال شغفك؟")
        interest = st.selectbox("", ["التقنية", "البيئة", "الصحة", "التعليم", "الطاقة"])
        if st.form_submit_button("استخراج الفكرة الريادية ✨"):
            st.success(f"فكرتكِ: ابتكار مشروع في مجال {interest} يعتمد على مهاراتكِ في مادة القوة!")
    st.markdown("</div>", unsafe_allow_html=True)
    st.write("ملاحظة: لقد تم حفظ نتائجكِ بنجاح في سجلات LAI.")
